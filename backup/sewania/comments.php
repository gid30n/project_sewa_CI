...
<div id="comments" class="comments-area">
    ...
    $fields = array(
        ...
        'email'  => '</div><div class="column column-medium-6"><p class="comment-form-email"><label for="email" class="screen-reader-text">' . esc_html__('Email', 'writsy') . '</label>' .
                    '<input type="email" id="email" name="email" value="' . esc_attr($commenter['comment_author_email']) . '" placeholder="' . esc_attr('Email Address', 'writsy') . '" aria-describedby="email-notes"' . $aria_req . $html_req  . '></p></div></div>',
        'url'    => '<p class="comment-form-url"><label for="url" class="screen-reader-text">' . esc_html__('Website', 'writsy') . '</label>' .
                    '<input type="url" id="url" name="url" value="' . esc_attr($commenter['comment_author_url']) . '" placeholder="' . esc_attr('Website', 'writsy') . '"></p>',
    );

    comment_form(array(
        'comment_notes_before' => null,
        'fields'               => $fields,
        'comment_field'        => '<p class="comment-form-comment"><label for="comment" class="screen-reader-text">' . esc_html__('Comment', 'writsy') . '</label> <textarea id="comment" name="comment" rows="5" placeholder="' . esc_attr('Type your comment here &hellip;', 'writsy') . '" aria-required="true" required="required"></textarea></p>',
