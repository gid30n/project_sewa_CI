<main>
	<div class="row">
		...
		<?php echo form_open_multipart('/peralatan-new/post');?>
							...
							<div class="container">
								...
								<div class="row">
									<div class="file-field input-field">
										<div class="btn">
											<span>File</span>
											<input class="file-upload" type="file" name="gallerys[]" multiple>
										</div>
			...
			$( "#provinsi option:selected" ).each(function(){
				...
				$.getJSON( "<?php echo base_url();?>api/region/"+val_provinsi, function( data ) {				
					for (var i = 0; i < data.length; i++) {
						kabupaten.push(data[i].name);
					}


				    var Kabupaten = new Bloodhound({
				        datumTokenizer: Bloodhound.tokenizers.whitespace,
				        queryTokenizer: Bloodhound.tokenizers.whitespace,	       
				        local: kabupaten
				    });

				    Kabupaten.initialize();

				    // $( "input" ).materialtags({
				    $('#lokasi .typeahead').materialtags({			    	   	
				        typeaheadjs: {        	       	       
				            source: Kabupaten
				        }
				    });
