...
<script>
	$(document).ready(function(){
		var total_record = 3;
 		var total_groups;
 		$.getJSON( '<?php if($level === "-9"){ echo base_url("api/all/ads/kategori-count/2"); }else{ echo base_url("api/user/ads/kategori-count/2");} ?>', function( data ) {
	 		...
	 		$(window).scroll(function(){
	 			if($(window).scrollTop() + $(window).height() == $(document).height()){
	 				if(total_record < total_groups){
	 					...
	 					$.getJSON('<?php if($level === "-9"){ echo base_url("api/all/ads/kategori/2/"); }else{ echo base_url("api/user/ads/kategori/2/");} ?>'+total_record+"/3", function(data){
	 						if (data != "") {
